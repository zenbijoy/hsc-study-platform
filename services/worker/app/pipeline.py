...
class ContentPipeline:
    ...
    def process(self, job: ImportJob) -> ImportJob:
        ...
        try:
            self.update(job, stage="quick_scan", progress=8, message="Fingerprinting source")
            ...
            self.update(job, stage="structure", progress=18, message="Extracting structure")
            if ext == ".pdf":
                ...
                logical_original = f"originals/{book_id}/{source.name}"
                self.update(job, stage="origin_upload", progress=31, message="Moving original to private origin storage")
                ...
                self.update(job, stage="pack", progress=48, message="Building encrypted HSCP package")
                ...
                if settings.content_master_key_b64:
                    ...
                    # Demo-only path. Production publication is blocked in docs until a master key is configured.
                    wrapped = {
                ...
                book_record = {
                    ...
                    "subtitle": "Imported by Content Factory",
                    "publisher": "Unspecified / verify rights",
                    "is_protected": True,
                    "is_published": False,
            ...
            self.update(job, stage="extract", progress=58, message="Normalizing, validating and deduplicating on disk")
            ...
            if stage_store.unique_count:
                self.update(job, stage="pack", progress=72, message="Building streamed content packs and FTS5 index")
            ...
            # Stage formula rows in the catalog in bounded batches. Heavy source data still lives
            # in object/content packs; this small catalog powers the native Formula UI.
            formula_catalog_staged = 0
            formula_batch: list[dict] = []
            for row in stage_store.iter_rows():
                if row["type"] != "formula":
                    continue
                ...
                formula_batch.append(payload)
                if len(formula_batch) >= 5000:
                    formula_catalog_staged += self.catalog.stage_formulas(formula_batch, job.id)
                    formula_batch.clear()
            if formula_batch:
                formula_catalog_staged += self.catalog.stage_formulas(formula_batch, job.id)

            self.update(job, stage="review", progress=92, message="Preparing confidence-based review summary")
            result = {
                "book": book_record,
                "packs": manifests,
                "duplicate_items_removed": stage_store.duplicates,
                "invalid_items": stage_store.invalid,
                "formula_catalog_staged": formula_catalog_staged,
                "review_required": True,
                "rights_check_required": True,
                "production_key_ready": bool(settings.content_master_key_b64),
                "ocr_recommended": bool(book_record and book_record.get("text_ratio", 1.0) < 0.35),
                "review_samples": stage_store.low_confidence(0.80, 100),
                "streaming_staging": {
            ...
            job.message = "Processing complete — review before publish"
            self.store.put(job)
            self.catalog.append(
                "imports",
                {
    ...
    def publish(self, job: ImportJob, rights_confirmed: bool) -> ImportJob:
        ...
        job.stage = "published"
        job.progress = 100
        job.message = "Published atomically"
