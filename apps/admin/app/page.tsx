import { BookOpenCheck, Database, HardDrive, Workflow } from 'lucide-react';
    ...
    <main className="mx-auto min-h-screen max-w-[1500px] px-5 py-7 lg:px-9 lg:py-9">
      <header className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-black tracking-[-.04em] md:text-5xl">Content Factory <span className="text-white/25">/ Admin</span></h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-white/45">One automated pipeline for books, formulas, CQ/MCQ, search packs, secure offline packages and AI-generated structured imports.</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3 text-xs text-white/45">Zero-cost dev mode · Local worker fallback</div>
      </header>
      ...
      <section className="mt-5 grid gap-5 lg:grid-cols-3">
        ...
        ].map(([title, body]) => <div key={title} className="rounded-[28px] border border-white/10 bg-white/[0.025] p-6"><h3 className="text-lg font-black">{title}</h3><p className="mt-3 text-sm leading-6 text-white/40">{body}</p></div>)}
      </section>
    </main>
  );
}
